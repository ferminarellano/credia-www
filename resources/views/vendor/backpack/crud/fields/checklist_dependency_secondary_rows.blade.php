...
<!-- checklist_dependency_secondary_rows -->
<div class="form-group col-md-12 checklist_dependency_secondary_rows"  data-entity ="{{ $field['field_unique_name'] }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <?php
      ...
      if( isset($id) && $id ){
		...
		/////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		$request_field_array = $field['subfields']['secondary'];
							...
							<td>
								<div class="checkbox">
									...
									<input type="checkbox"
										...
										data-id = "{{ $datos_key_value[$fila.'--'.$columna] }}"
										...
										@if( ( isset($field['value']) && is_array($field['value']) && (  in_array($datos_key_value[$fila.'--'.$columna], $field['value'][1]->pluck('id', 'id')->toArray()) || isset( $secondary_ids[$datos_key_value[$fila.'--'.$columna]])) || ( old($secondary_dependency['name']) &&   in_array($datos_key_value[$fila.'--'.$columna], old($secondary_dependency['name'])) )))
											 checked = "checked"
											 @if(isset( $secondary_ids[$datos_key_value[$fila.'--'.$columna]]))
											  disabled = disabled
											 @endif
										@endif />
									</label>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    <!-- include checklist_dependency js - modified checklist_dependency_secondary_rows-->
    <script>
      jQuery(document).ready(function($) {
        $('.checklist_dependency_secondary_rows').each(function(index, item){
